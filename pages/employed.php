...
<head>
    ...
    <style>
        ...
        .choice-card {
            ...
            text-align: center; padding: 1.5rem; border-radius: 16px; color: #666; height: 100%;
        }

        .hidden-radio:checked + .choice-card {
            border-color: #3a82f6; background: rgba(58, 130, 246, 0.15);
            color: #fff; transform: scale(1.05); font-weight: 800;
...
<body>
    ...
    <form id="employmentForm" class="px-6 h-screen flex items-center">
        ...
        <div class="step-container active" id="step-0" data-required="company_name">
        ...
        <div class="step-container" id="step-1" data-required="company_address">
        ...
        <div class="step-container" id="step-2" data-required="position">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-5 w-full custom-scroll overflow-y-auto max-h-[55vh] p-4">
                <?php
                $positions = ["IT Academician / Teacher", "Programmer / Developer", "Network Administrator", "IT Support Specialist / VA", "IT Engineer", "IT Analyst", "IT Quality Control", "IT Researcher", "IT Technician", "IT Consultant", "Computer Forensic / Investigator"];
                foreach($positions as $pos) {
                    echo "<label class='block h-24'>
        ...
        <div class="step-container" id="step-3" data-required="emp_status">
        ...
        <div class="step-container" id="step-4" data-required="date_from">
                ...
                <div class="flex flex-col md:flex-row gap-10 justify-center items-center mt-6">
                    ...
                    <div class="w-full md:w-1/3 text-center relative">
                        ...
                        <label class="flex items-center justify-center gap-2 mt-4 cursor-pointer">
                            <input type="checkbox" id="present_checkbox" name="is_present" class="accent-blue-500">
        ...
        <div class="step-container" id="step-5">
            ...
            <div class="flex gap-10">
                <button type="button" onclick="validateAndFinalize('YES')" class="w-56 py-6 bg-blue-600 rounded-3xl font-black text-2xl hover:scale-110 transition-transform shadow-2xl">YES</button>
                <button type="button" onclick="validateAndFinalize('NO')" class="w-56 py-6 border-2 border-gray-800 rounded-3xl font-black text-2xl hover:bg-white hover:text-black transition-all">NO</button>
            </div>
        </div>
    ...
    <script>
        ...
        function validateStep() {
            ...
            if (!requiredField) return true; // Step 5 has no specific field to validate before click

            if (requiredField === 'position') {
                const radioSelected = currentStep.querySelector('input[name="position"]:checked');
                const otherInput = document.getElementById('other_pos_input').value.trim();
                if (!radioSelected && !otherInput) isValid = false;
            } 
            else if (requiredField === 'emp_status') {
                isValid = currentStep.querySelector('input[name="emp_status"]:checked') !== null;
            }
            else {
            ...
            if (!isValid && (requiredField === 'position' || requiredField === 'emp_status')) {
                currentStep.classList.add('animate-shake');
                setTimeout(() => currentStep.classList.remove('animate-shake'), 400);
            }
        ...
        // Date Checkbox Logic
        document.getElementById('present_checkbox').addEventListener('change', function() {
            const dateTo = document.getElementById('date_to');
            dateTo.disabled = this.checked;
            dateTo.style.opacity = this.checked ? "0.3" : "1";
            if(this.checked) dateTo.value = "";
        });

        // Other Position Input - Unselect radios when typing
        ...
        function validateAndFinalize(choice) {
            // Collect data and save to localStorage or redirect
            const formData = new FormData(form);
            const data = Object.fromEntries(formData.entries());
            
            // Merge with existing tracer data
            let existingData = JSON.parse(localStorage.getItem('tracer_payload')) || {};
            let finalPayload = { ...existingData, ...data, has_previous_exp: choice };
